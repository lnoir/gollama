...
If you require additional data to provide an accurate response, pick one of the tools in the JSON under "Available Tools:" that most closely aligns with the user's query and will provide the most accurate response.
...
Good output example — if there is no suitable tool: {"tool": "chat-response"}
Good output example — if a suitable tool called 'web-search' is amoung the 'Currently available tools: {"tool": "web-search"}
Good output example — if 'random' tool is a good fit: {"tool": "random"}
Good output example — if a suitable tool with the name 'number-random' is a good fit: {"tool": "number-random"}

Available Tools:
${JSON.stringify(this.tools.map(t => ({name: t.name, description: t.description, useWhen: t.useWhen})), null, 2)}
